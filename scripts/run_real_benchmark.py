...
---------------------
Standalone script: loads the real Porto Taxi dataset, runs the full 5-index
CRI benchmark, and stores results to output/real_benchmark/.

Usage:
    python scripts/run_real_benchmark.py
    python scripts/run_real_benchmark.py --n-points 10000 --n-queries 2000
    python scripts/run_real_benchmark.py --n-points 100000 --out-dir output/scale/100000
    python scripts/run_real_benchmark.py --grid-dims 4,4
"""
...
_parser = argparse.ArgumentParser(description="VLUI real-dataset CRI benchmark")
...
_parser.add_argument("--out-dir",    type=str,   default="",      help="Output directory (default output/real_benchmark)")
_parser.add_argument("--dataset",    type=str,   default="",      help="Path to train.csv (default data/porto_taxi/train.csv)")
...
WINDOW_SIZE  = 50
OUT_DIR      = Path(_args.out_dir) if _args.out_dir else PROJECT_ROOT / "output" / "real_benchmark"
...
print()
print("╔══════════════════════════════════════════════════════╗")
print("║   VLUI — Real Dataset Benchmark (Porto Taxi)         ║")
print("╚══════════════════════════════════════════════════════╝")
...
# Per-window snapshot series (for CSV export)
snapshots: list[dict] = []

# Scale snapshot frequency so there are always ~200 snapshots regardless of n
...
# 2. CSV — one row per snapshot window
...
# 3. CSV — final summary (one row per index)
...
print(f"\n  Results saved to  output/real_benchmark/")
